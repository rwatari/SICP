...
(define time-to-height
  (lambda (vertical-velocity elevation target-elevation)
    (root2 (- (/ gravity 2)) vertical-velocity (- elevation target-elevation))))
...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    ...
    (let ((rad-ang (degree2radian angle)))
      (* velocity
         (cos rad-ang)
         (time-to-impact (* velocity (sin rad-ang)) elevation)))))
...
;;; increment in 1 degree

(define find-best-angle
  (lambda (velocity elevation)
    ;;; takes starting velocity and elevation
    ;;; returns angle of farthest distance
    (define (iter guess-angle best-angle best-distance)
      ;;; recursive function to find best angle, incrementing guess by alpha-increment
      (let ((guess-distance (travel-distance-simple elevation velocity guess-angle))
            (next-angle (+ guess-angle
                           alpha-increment)))
        (if (> guess-angle 90) ; if finished with rangle of angles, return best
            best-angle
            (if (> guess-distance best-distance) ;else call iter again
                (iter next-angle          ;if current guess is better than previous
                      guess-angle         ;replace best guess values
                      guess-distance)
                (iter next-angle
                      best-angle
                      best-distance)))))
    (iter 0 0 0))) ;initialization starting at 0 degrees
;; find best angle
...
(define integrate
  (lambda (x0 y0 u0 v0 dt g m b)
    (define (iter x y u v)
      (if (<= y 0)
          x
          (let ((vel (* (sqrt (+ (square u)
                              (square v)))
                        b
                        (/ 1 m))))
            (iter (+ x (* u dt))
...
(define travel-distance
  (lambda (elevation velocity angle)
    ...
    (let ((rad-angle (degree2radian angle)))
      (integrate 0 
                 elevation
                 (* (cos rad-angle) velocity)
                 (* (sin rad-angle) velocity)
                 .01
                 gravity
                 mass
                 beta))))
