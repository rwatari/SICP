...
(define (add-to-index! index key value) ; Index,Key,Val -> Index
  (let ((index-entry (find-entry-in-index index key)))
	...
	(let ((new-entry (cons (list key (list value))
                               '())))
          (set-cdr! new-entry (cdr index))
          (set-cdr! index new-entry))

      ;; entry exists -- insert value if not already there...
	(if (not (memq value (cadr index-entry)))
            (let ((new-values (cons value (cadr index-entry))))
              (set-car! (cdr index-entry) new-values)))
	))
  ;index
  )
