...
(define (solve-ax+by=1 a b)
  (let ((q (quotient a b))
    ...
    (if (= b 1)
        (cons 0 1)
        (let ((pair (solve-ax+by=1 b r)))
          (cons (cdr pair)
                (- (car pair) (* q (cdr pair))))))))
                     
#|
a=bq+r
(bq+r)x + by = b(qx+y) + rx = 1 = bx' + ry' -> x = y' y = x'-qy'
...
(define (RSA-unconvert-list intlist key2)
  (let ((n (key-modulus key2)))
    (define (unconvert l sum)
      (if (null? l)
          '()
          (let ((x (RSA-transform (modulo (car l) n) key2)))
            (cons (modulo (+ x sum) n) (unconvert (cdr l) (car l))))))
    (unconvert intlist 0)))
...
(define get-signature cdr)
;;;signed messages are pairs

(define (encrypt-and-sign message-string sender-private-key recipient-public-key)
  (let ((message (RSA-encrypt message-string recipient-public-key)))
    (make-signed-message message
                         (RSA-transform (modulo (compress message)
                                                (key-modulus sender-private-key))
                                  sender-private-key))))

(define (authenticate-and-decrypt signed-message sender-public-key recipient-private-key)
  (let ((message (get-message signed-message))
        (signature (get-signature signed-message)))
    (if (not (= (RSA-transform (modulo signature (key-modulus sender-public-key))
                               sender-public-key)
                (compress message)))
        #f
        (RSA-decrypt message recipient-private-key))))
...
(define public-key-list
  (list bill-clinton-public-key
        al-gore-public-key
        bob-dole-public-key
...
(define (message-cracker)
  (define (iter list)
    (let ((sender-key (car list)))
      (if (not (authenticate-and-decrypt (cons received-mystery-message received-mystery-signature)
                                         sender-key
                                         newt-gingrich-private-key))
          (iter (cdr list))
          sender-key)))
  (iter public-key-list))
