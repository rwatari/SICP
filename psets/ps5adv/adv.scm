...
(define (all-prerequisites entry)
  (define (rec subject-list)
    (reduce list-union
            '()
            (map  (entry-prerequisites subject))))
  (rec (list (entry-subject entry))))
...
(define subject-knowledge 
  (list
   ...
   (make-rule
    `(what are all prerequisites for (?? s ,subjects))
    (lambda (dict)
      (for-each (lambda (entry)
		  (write-line
		   (append '(the prerequisites for)
