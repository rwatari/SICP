...
(define (make-student-residence name)
  (let ((card-place (make-card-locked-place name))
        (registered-ids '()))
    (lambda (message)
      (cond ((eq? message 'accept-person?)
             (lambda (self person)
            ...
            ((eq? message 'register-card)
             (lambda (self sd-card)
               (cond ((not (eq? self
                                (ask sd-card 'place)))
                      (display-message (list (ask sd-card 'name) "is not in" name)))
                     ((not (is-a sd-card 'sd-card?))
                      (display-message (list "cannot register" (ask sd-card 'name)
                                             "in system")))
                     (else (set! registered-ids (cons (ask sd-card 'id)
                                                      registered-ids))
    ...
    (lambda (message)
      (cond ((eq? message 'act)
             (lambda (self)
               (let ((others (other-people-at-place self (ask self 'place))))
                 ;;;---------------------------
                 ;;; need to check here if others has a person with 
                 ;;; a stolen id
                 (if (not (null? others))
